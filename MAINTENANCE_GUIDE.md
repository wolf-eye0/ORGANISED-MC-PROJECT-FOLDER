...
2. Latest accepted Decision Register—currently v1.3.
...
- `07_SEMESTER_EXECUTION/`: current VibeGuard Phase 4 execution area; keep live operational documents in their established `CURRENT` locations.
...
- Decision Register v1.3 is the current controlled register and remains immutable during ordinary documentation maintenance.
- Decision Register v1.2 and v1.0 remain preserved historical predecessors.
- Create a later version only after a real new governing decision; never edit v1.3 in place to hide chronology.
- A later version must state what changed, inherit accepted history and must not silently renumber PDR entries.

## 21. Teacher-approval workflow

1. Preserve the owner-reported verbal delegation already recorded in current governance without fabricating a written approval artifact.
2. The team-selected VibeGuard decision is controlled by the Final Selection and Phase 4 Entry Memo and Decision Register v1.3.
3. Capture any later faculty conditions, rejection or approval evidence with date and provenance.
4. Never invent a signature, email, quotation or institutional memo.
5. Record later changes in a new controlled decision version rather than rewriting the historical request or v1.3.

## 22. Phase-4 entry workflow

Phase 4 is open for the final VibeGuard semester selection. Current project-state controls are Decision Register v1.3 and the Final Selection and Phase 4 Entry Memo.

- Use `07_SEMESTER_EXECUTION/01_Selected_Project/CURRENT/` for the package index, Primer and Playbook.
- Use `07_SEMESTER_EXECUTION/02_BOM_and_Procurement/CURRENT/` for current procurement/acceptance control.
- Use `07_SEMESTER_EXECUTION/03_Hardware_and_CAD/` for controlled hardware drawings, CAD, wiring and as-built records.
- Use `07_SEMESTER_EXECUTION/04_Firmware/` for controlled firmware source, releases and configuration evidence.
- Use `07_SEMESTER_EXECUTION/05_Test_Protocols_and_Data/` for current protocols, immutable raw data, processed derivatives and run evidence.
- Use `07_SEMESTER_EXECUTION/06_Reports_and_Demonstration/` for current reports, demonstration assets and presentation releases.
- Use `07_SEMESTER_EXECUTION/99_SUPERSEDED_PHASE4_DRAFTS/` for preserved superseded Phase 4 drafts.
- Keep PIRG and other post-semester work in concept future-research areas unless a later controlled decision opens a dedicated execution scope.

Procurement preparation is authorized, but a BOM does not prove that purchasing occurred. Refresh seller price/availability and acceptance evidence at purchase time.
...
- [ ] Decision Register v1.3, Final Selection Memo and final architecture hashes are unchanged.
...
- [ ] Teacher delegation and team selection are described exactly as current governance records them.
- [ ] Phase 4 is correctly shown open; procurement is not falsely shown completed.
...
- Never create a later Decision Register or Master Manual release without an authorized decision/release event.
...
## 27. Live navigation maintenance

The current navigation layer is:

- `README.md` — status and top-level map;
- `00_START_HERE/WHERE_TO_LOOK.md` — task-oriented routing;
- `FILE_CATALOG.md` — exhaustive human-readable physical-file reference;
- `00_START_HERE/CURRENT_FILE_INVENTORY.tsv` — machine-readable inventory;
- `MAINTENANCE_GUIDE.md` — maintenance policy.

Whenever a material controlled file is added, moved or versioned, update:

1. the relevant package/index file;
2. `FILE_CATALOG.md` and the machine-readable inventory;
3. `WHERE_TO_LOOK.md` only if the human navigation route changes.

Raw experimental data is immutable. Negative results remain linked to their run/configuration identities. Seller-price snapshots are dated evidence rather than permanent prices. Never overwrite current files to simplify navigation; archive the predecessor and preserve its hash.
