### Project mC Decision Register — Version 1.1

**Document Control Header**
*   **Version:** 1.1
*   **Current project stage:** Phase 3C in progress
*   **Phase 3C completed concepts:** OpenBraille, VibeGuard, and TrueMoist
*   **Phase 3C pending concepts:** TrustLatch and ColdTrace
*   **Authoritative as of:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md` and `Phase3C_TrueMoist_MEMORY.md`
...
**Phase introduced:** Phase 2 / Phase 3C
**Category:** Portfolio & Commercialization & Patent Strategy
**Decision or requirement:** Confine all TrueMoist IP efforts strictly to the edge-AI algorithmic software layer, utilizing unmodified off-the-shelf commodity sensors. Phase 3C extends this explicitly to exclude custom live-EC electrode hardware from the MVP runtime scope, not just custom moisture-probe hardware.
**Rationale:** Designing custom probes (or custom EC electrode circuits) with integrated EC/Temp/Capacitance directly infringes on aggressively defended utility patents by Toro, Meter Group, and Campbell (incl. US 11,598,743, active to ~2040).
**Source evidence:** `Phase2_Report by CLaude.md`, `QWen Research on Patent.pdf`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Patenting the physical hardware probe; live custom EC electrode circuit.
**Decision type:** Recommendation
**Verification status:** Verified
**Current status:** Reaffirmed and extended
**Carry-forward requirement:** Yes
...
**Phase introduced:** Phase 3A / Phase 3B / Phase 3C
**Category:** Validation, Testing & Feasibility
**Decision or requirement:** Require gravimetric analysis (oven-drying soil samples at 105°C for ≥24h to constant mass) as the absolute baseline for validation. Phase 3C finalizes the full procedure: 0.01g preferred scale resolution (1g fallback documented as increasing uncertainty), dehydrator/microwave as explicitly lower-standing fallbacks, not substitutes of equal standing.
...
**Source evidence:** `Phase3B_Report_by_Claude.md`, `Claudes report on the research Phases 3a.pdf`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Validating against commercial IoT sensors.
**Decision type:** Requirement
**Verification status:** Verified
**Current status:** Accepted — finalized
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-012, PDR-031, PDR-061
...
**Decision or requirement:** Utilize offline-trained, ridge-regularized 2nd-order multivariate polynomial regression (Raw capacitance + Temperature + coded Salinity class) to correct raw capacitive moisture readings, deployed as static embedded inference.
**Rationale:** Corrects for Maxwell-Wagner interfacial polarization and ionic drift without requiring cloud connectivity, purely in software; ridge regularization mitigates overfitting risk given the ~135-sample calibration dataset.
**Source evidence:** `Phase3B_Report_by_Claude.md`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Threshold-based control, AD5933 Impedance Analyzer, unregularized regression, TinyML/adaptive recalibration.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-045, PDR-049, PDR-061, PDR-067

**Decision ID:** PDR-032
**Title:** TrueMoist ADC Selection
**Phase introduced:** Phase 3B / Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Mandate the use of an external 16-bit ADC (ADS1115) by default, confirmed rather than decided by a Week-1 internal-vs-external comparison gate.
**Rationale:** Error-budget analysis shows the ESP32's internal ADC has an effective ~9-bit resolution (~6.4mV error band) against a 10–50mV compensation-relevant signal — comparable orders of magnitude that risk the regression fitting acquisition noise. The ADS1115 costs ~₹120–150, trivial against the budget ceiling.
**Source evidence:** `Phase3B_Report_by_Claude.md`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Native MCU ADC only.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted — resolved
**Carry-forward requirement:** Yes
...
**Phase introduced:** Phase 3B / Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Restrict the prototype calibration scope to a single, homogeneous, sieved local soil type (e.g. sandy loam or loam), documented once per calibration batch.
...
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-012, PDR-061

**Decision ID:** PDR-050
**Title:** TrueMoist AFE Soldering
**Phase introduced:** Phase 3B / Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Breadboard permitted only for Week 1 characterization; solder the analog front-end connections to perfboard from Week 2 onward.
...
**Source evidence:** `Phase3B_Report_by_Claude.md`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Solderless breadboard prototyping throughout.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-058
**Title:** TrueMoist EC/Salinity Strategy
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Represent salinity as a controlled calibration variable only (known NaCl-solution classes during dataset preparation); no live runtime soil-EC channel in the MVP.
**Rationale:** Live EC sensing requires AC bipolar-pulse excitation to avoid electrode polarization/electrolysis — an analog-EE problem outside the team's stated skill gap; commercial EC probes alone risk the budget ceiling; keeps IP off custom sensing hardware per PDR-045.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Live two-electrode galvanic EC (Mistral's minority position); AD5933 frequency-domain impedance analysis.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-031, PDR-045

**Decision ID:** PDR-059
**Title:** TrueMoist Moisture Sensor Selection
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Commodity capacitive soil-moisture probe (v2.0/YL-69/HL-69-class), analog output; 2 units purchased to characterize sensor-to-sensor variability before full dataset collection.
**Rationale:** Cheap, epoxy-shielded (low corrosion risk vs. resistive probes), and its known temperature/salinity sensitivity is precisely the error surface the compensation algorithm exists to correct.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Resistive probe; custom probe.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-060
**Title:** TrueMoist Temperature Sensor & Placement
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Waterproof DS18B20, soil-depth co-located ~1–2cm from the moisture probe; ambient-air sensing explicitly rejected as a substitute.
**Rationale:** Soil's thermal inertia means ambient air lags actual soil temperature, breaking the correlation the compensation model depends on.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Ambient-air temperature sensing; TMP36 analog sensor.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-061
**Title:** TrueMoist Calibration Dataset Design
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** 5 moisture levels × 3 temperature levels × 3 salinity levels × 3 replicates = 135 samples; grouped train/validation split (~100/~35) by physical batch, plus one held-out (leave-one-condition-out) block.
**Rationale:** Convergent mid-point across four independent proposals (72–180 samples); preserves full 3-level coverage on both named confounders (temperature, salinity) rather than cutting either for convenience.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Perplexity's reduced 72-sample plan (2 temperature levels); Gemini's 180-sample plan (4 replicates).
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-031, PDR-049, PDR-048

**Decision ID:** PDR-062
**Title:** TrueMoist Validation Metrics & Success Threshold
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** RMSE as primary metric (MAE, R², disturbance-sensitivity as secondary); success threshold ≥25% RMSE reduction target (≥20% minimum floor), compensated vs. uncompensated, against the same gravimetric reference.
**Rationale:** RMSE selected by 3-of-4 reports (Gemini, Perplexity, ChatGPT); 25% target sits inside every individual report's proposed range (20–30%).
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Mistral's MAE-primary framing with an Improvement Ratio >1.2× threshold (retained as an equivalent secondary check).
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-031, PDR-048

**Decision ID:** PDR-063
**Title:** TrueMoist Mechanical Test Rig Standard
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Rigid plastic container (~500mL–1L, 8–12cm diameter), fixed-depth insertion jig, mass-controlled compaction, 20–30 min equilibration, unique per-batch labeling traceable to gravimetric measurement.
**Rationale:** Minimizes soil-packing and insertion variability, the two highest cross-report-flagged physical risks after gravimetric quality itself.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Deformable containers; unguided free-hand probe insertion.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-049

**Decision ID:** PDR-064
**Title:** TrueMoist Interface & Logging
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** USB serial CSV mandatory; 0.96" I²C OLED optional for live demonstration only; no SD card, no cloud connectivity.
**Rationale:** Prioritizes evidence collection and interpretability over interface complexity; avoids gold-plating the academic prototype.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** SD-card logging; smartphone app; cloud dashboard.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-065
**Title:** TrueMoist Power Architecture
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** USB 5V benchtop power only; no battery for the MVP.
**Rationale:** Battery behavior adds no evidentiary value to proving the compensation hypothesis while introducing brownout-analysis risk and falling-voltage measurement anomalies.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Li-ion battery + TP4056 charging module.
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-066
**Title:** TrueMoist Phase 3C Architecture Selection
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Primary architecture = offline-trained/embedded-inference (Architecture D) hardened with an external ADC (Architecture B-style), controlled-salinity EC strategy. Fallback = Architecture A (temperature-only, internal ADC) if Week 1–2 validation gates fail.
**Rationale:** D+B-hardened dominates on team-fit, validation clarity, and overfitting risk across the comparative decision matrix; Architecture C (adaptive recalibration) rejected for the two-month MVP on overfitting/timeline grounds.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Architecture B (live EC); Architecture C (adaptive on-device recalibration).
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-031, PDR-032, PDR-058

**Decision ID:** PDR-067
**Title:** TrueMoist Compensation Model Regularization
**Phase introduced:** Phase 3C
**Category:** Concept-Specific Architecture (TrueMoist)
**Decision or requirement:** Adopt ridge regularization on the 2nd-order polynomial regression, over an unregularized fit.
**Rationale:** ~135-sample dataset relative to a ~10-coefficient 2nd-order model with cross-terms carries real overfitting risk that three of four reports flagged as Medium but did not fully mitigate in their own proposed architectures.
**Source evidence:** `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Unregularized 2nd-order regression (Mistral); linear-only baseline (Perplexity default).
**Decision type:** Final decision
**Verification status:** Verified
**Current status:** Accepted
**Carry-forward requirement:** Yes
...
**Decision ID:** PDR-068
**Title:** TrueMoist Patent Scope Refinement
**Phase introduced:** Phase 3C
**Category:** Portfolio & Commercialization & Patent Strategy
**Decision or requirement:** Confirm the Phase 3C MVP (static ridge-regularized regression) is not independently patentable; explicitly carry forward on-node adaptive self-calibration (Architecture C) as the concept's genuine future patent path, deferred to Phase 6+/a post-MVP validation sprint rather than abandoned.
**Rationale:** Phase 2's independent, two-source-corroborated finding (Kimi and Gemini) is that self-updating recalibration is the only defensible IP white space; the hardware-compensation-circuit space (including this MVP's shape) is already saturated (US 11,598,743 and the Toro/Meter/Campbell estate).
**Source evidence:** `Phase2_Report by CLaude.md`, `Phase3C_TrueMoist_Architecture_Report_by_Claude.md`
**Alternatives:** Treating the MVP as sufficient IP in itself; abandoning the patent question entirely.
**Decision type:** Recommendation / carried-forward open question
**Verification status:** Partially Verified
**Current status:** Open question — carried forward
**Carry-forward requirement:** Yes
**Related PDR entries:** PDR-045, PDR-012
