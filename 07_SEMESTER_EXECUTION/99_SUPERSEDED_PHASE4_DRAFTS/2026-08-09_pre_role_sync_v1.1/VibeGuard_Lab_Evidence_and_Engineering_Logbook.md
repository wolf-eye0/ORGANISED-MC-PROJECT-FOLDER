...
**Document status:** Ready-to-use logging system, revision 1.0  
...
|---|---|---|
| **Nihad P C** | Architecture/configuration status, decisions, interface changes, integration failures, risk, acceptance gates, weekly freeze and evidence integrity | Reviews final links/sign-offs; keeps backup access current. |
| **Sreehari K** | Firmware/commits, sensor configuration, sample/drop evidence, feature definitions, RMS/persistence, FFT, USB schema, scripts, splits and metrics | Nihad reviews releases/parameters; Amith reviews hardware assumptions. |
| **Amith Krishna Das** | Procurement, part/batch IDs, power/wiring, motor rig, mounts, dimensions/fasteners, safety, hardware failures/repairs and before/after photos | Nihad reviews interfaces; a second person observes energized rig tests. |
| **Sreeprada K S** | Inventory, operator/run checklist, repeated sessions, remount photos, labels, run IDs, learning notes, observed failures and procedure-conformance confirmation | **Lab Assistant / Technical Observer.** Does not independently alter threshold, wiring, safety feature or firmware release. |
| **Archa Pramod** | Minutes, document revision, teacher communication, source/evidence index, photo/video catalog, weekly summary, demo script, slides and report assembly record | **Lab Assistant / Technical Observer.** Technical claims require technical-member review. |
