...
**Document status:** Semester execution baseline, revision 1.0  
...
|---|---|---|---|
| **Nihad P C** | Project/integration owner; architecture/configuration control; interfaces; risk; evidence integrity; acceptance gates | Sreehari backs up firmware/configuration; Amith backs up hardware/safety; Nihad must keep handover material current | Technical owner; does not self-approve all work—another named reviewer signs each gate. |
| **Sreehari K** | Firmware, SPI acquisition, DSP, USB schema, analysis scripts, data splits and metrics | Nihad reviews releases/parameters; Amith checks hardware assumptions; Sreehari teaches a second member each critical procedure | Technical member; owns no unsafe mechanical change without Amith review. |
| **Amith Krishna Das** | Procurement, component IDs, power/wiring, motor rig, sensor mounting and safety | Nihad reviews architecture interfaces; Sreehari checks signals/pinout; another person observes every energized rig gate | Technical hardware/safety lead. |
| **Sreeprada K S** | Learner/operator, inventory, procedural run records, labels, remount evidence and observed failures | Amith or Sreehari reviews technical entries; Nihad approves any proposed change | **Lab Assistant / Technical Observer.** No unsupervised wiring, threshold, firmware-release or mechanical-safety change. |
| **Archa Pramod** | Minutes, controlled documents, teacher communication, source/evidence index, demonstrations and final-report assembly | Technical claims reviewed by Nihad, Sreehari or Amith; another member backs up document archive | **Lab Assistant / Technical Observer.** No unsupervised critical hardware, firmware, algorithm or safety work. |
...
- [ ] Sreehari explains sampling, RMS, persistence and data leakage using one example each.
- [ ] Amith explains motor isolation, guard, emergency disconnect and power-off adjustment.
- [ ] Sreeprada demonstrates how to assign IDs and fill a run checklist under supervision.
- [ ] Archa creates the controlled meeting record and source index.
...
**“Can Sreeprada or Archa make a quick threshold/wiring change?”**  
They can record and propose a change. A technical owner must implement/review it through change control; safety-critical work is never unsupervised.
